...
class PostgresDB:
    ...
    def merge_data(self, table_name: str, table_schema: list, data: Union[list, dict]):
        cursor = self.conn.cursor()
        ...
        merge_query = self._prepare_merge_query(cursor, table_name, table_schema, final_data)

        cursor.execute(merge_query)
        self._logger.info(f'Affected rows: {cursor.rowcount}')
        self.conn.commit()

    def sync_data_to_postgres(self, table_name: str, table_schema: list, data: Union[list, dict]):
        ...
        self.merge_data(table_name, table_schema, data)
